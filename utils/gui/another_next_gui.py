...
# --- Training Loop ---
def training_loop(manager: TrainingManager, max_epochs=9999):
    device = "cpu"
    ...
    for epoch in range(max_epochs):
        for batch_idx in range(20):
            ...
            manager.update_info(loss=loss.item(), image=out[0])
            time.sleep(0.05)
...
class HumanFeedbackWidget(QWidget):
    ...
    def generate_samples(self):
        self.labels = [f"Label {i}" for i in range(self.max_images)]
        self.images = [torch.rand(3,64,64) for _ in range(self.max_images)]
        for i, (label, img_tensor) in enumerate(zip(self.labels, self.images)):
            label_widget, img_widget = self.image_widgets[i]
            label_widget.setText(label)
            img = np.ascontiguousarray((img_tensor.permute(1,2,0).numpy()*255).astype(np.uint8))
            h,w,ch = img.shape
            bytes_per_line = ch*w
    ...
    def send_feedback(self):
        ...
        self.labels = []
        self.images = []
        for label_widget, img_widget in self.image_widgets:
...
class TrainingGUI(QWidget):
    def __init__(self, manager: TrainingManager):
        super().__init__()
        self.manager = manager
        self.hf_widget = HumanFeedbackWidget(manager)
        self.init_ui()
        self.start_timer()
